    ...
    <div class="row">
      <?php $style = array(0 => 'skew-shadow', 1 => 'bubble-shadow', 2=> 'curves-shadow'); foreach ($promo as $item): //if($item->status==0){continue;} ?>
        <div class="col-md-4">
          ...
          <div class="card card-dark bg-<?php if($item->usage==0 && $item->status==1){echo 'success';}elseif($item->available==0 or $item->deadline<=0 or $item->status==0 ){echo 'danger';}elseif($item->avability<=(($item->qty)/4)){echo 'warning';}else{echo 'secondary';}  ?>-gradient">
